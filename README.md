...
The codebase is deliberately structured around SOLID principles. Here is how each principle maps to actual classes and methods.
...
|---|---|
| `WorkflowService` (`challenge-core`) | Tree traversal — follows `yes`/`no` branches until an `ActionNode` is reached. It knows nothing about notifications or storage. |
| `NotificationService` (`challenge-core`) | Dispatching approval requests via the correct channel (`sendViaSlack` / `sendViaEmail`). It knows nothing about the workflow tree. |
| `InMemoryWorkflowRepository` (`challenge-core`) | Storing and retrieving the workflow root node. It knows nothing about evaluation logic. |
| `AmountGreaterThan`, `DepartmentEquals`, `RequiresManagerApproval` (`challenge-data`) | Each condition class owns its own evaluation logic via `evaluate(invoice)`. None of them know about the tree or notifications. |
| `WorkflowSeeder` (`challenge-core`) | Constructing and returning the Fig. 1 `WorkflowNode` tree. It has no runtime responsibilities. |

### Open/Closed

Classes are open for extension, closed for modification:

- **New condition types:** Add a subclass of `Condition` (e.g. `AmountLessThan`) with its own `evaluate()` override in `challenge-data/WorkflowNode.kt`. `WorkflowService.traverse()` calls `node.condition.evaluate(invoice)` — it never needs to change.
- **New notification channels:** Add a value to `NotificationChannel` and a new branch in `NotificationService.send()`. The workflow engine (`WorkflowService`) and domain model (`ActionNode`) are untouched.

### Liskov Substitution

Subtypes are fully substitutable for their supertypes:

- `ConditionNode` and `ActionNode` both extend `WorkflowNode`. `WorkflowService.traverse()` accepts a `WorkflowNode` and handles any subtype correctly via a sealed `when`. No subtype breaks the contract.
- `InMemoryWorkflowRepository` implements `WorkflowRepository`. Swapping in a `PostgresWorkflowRepository` requires no change to `WorkflowService`, `App.kt`'s wiring logic, or any test that injects the interface.

### Interface Segregation

`WorkflowRepository` (in `challenge-data`) exposes exactly two methods:
...
`WorkflowService` only calls `getWorkflowRoot()`. `App.kt` only calls `saveWorkflow()`. Neither consumer is forced to depend on storage internals, cache strategies, or schema details.

### Dependency Inversion

`WorkflowService` depends on the `WorkflowRepository` *interface*, not on `InMemoryWorkflowRepository` directly:

```kotlin
class WorkflowService(
    private val repository: WorkflowRepository,        // ← interface
    private val notificationService: NotificationService
)
```

The concrete `InMemoryWorkflowRepository` is instantiated only in `App.kt` (the composition root). In tests, `WorkflowRepository` is substituted with a MockK mock — `WorkflowService` never needs to know which implementation it is talking to.
